  ...
  <div class="homepage_container">
    <div class="homepage_header">
      ...
      <el-breadcrumb separator="/" class="bread">
        <el-breadcrumb-item v-for="(item, index) in matchedRoute" :key="index" :to="(index < matchedRoute.length-1 ? { path: `/${item.toLowerCase()}` } : {})">{{item}}</el-breadcrumb-item>
      </el-breadcrumb>
      <div class="right">
    ...
    <div class="homepage_menu">
      <el-menu :default-active="defaultActive" class="el-menu-vertical-demo" :router="true" :unique-opened="false">
        <template v-for="item in menus" :key="item.path">
          ...
          <!-- children只有一项 -->
          <el-menu-item :index="item.path !== '/' ? item.path +'/'+ item.children[0].path: '/' + item.children[0].path" v-else-if="item.children && item.children.length === 1 &&!item.meta">
