<template>
  <div class="detail">
    ...
    <section class="con">
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <span v-if="categoryView.category1Id">{{
          categoryView.category1Name
        }}</span>
        <span v-if="categoryView.category2Id">{{
          categoryView.category2Name
        }}</span>
        <span v-if="categoryView.category3Id">{{
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <MyZoom :skuImageList="skuImageList" />
          <!-- 小图列表 -->
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="s in spuSaleAttrList" :key="s.id">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: +spuVal.isChecked }"
                  v-for="spuVal in s.spuSaleAttrValueList"
                  :key="spuVal.id"
                  @click="changeActive(spuVal, s.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="mins"   @click="changeNum(-1)" :class="{disabled: skuNum < 2}">-</a>
              </div>
              <div class="add">
...
import MyZoom from "./MyZoom/MyZoom";
...
export default {
  ...
  methods: {
    // 产品的售卖属性值切换高亮
    changeActive(spuVal, spuSaleAttrValueList) {
      spuVal.isChecked = +spuVal.isChecked ? '0' : '1'
      spuSaleAttrValueList.forEach(s => {
        if (s !== spuVal) {
          s.isChecked = '0'
        }
      })
    ...
    async addShopCart() {
      ...
      try {
        await this.$store.dispatch('addOrUpdateShopCart', payload)
        // 成功后进行路由跳转
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo))
        // const temp = JSON.parse(sessionStorage.getItem('SKUINFO'))
        const location = {
          path:'/addcartsuccess',
          query:{
            skuNum: this.skuNum,
